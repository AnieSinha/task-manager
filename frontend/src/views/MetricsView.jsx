...
export function MetricsView() {
  ...
  const summary = [
    ...
    {
      label: "Features",
      value: counts.features.total,
      sub: `${counts.features.completed} completed`,
    },
    {
      label: "Stories",
      value: counts.stories.total,
      sub: `${counts.stories.inProgress} in progress`,
    },
    {
      label: "Tasks",
      value: counts.tasks.total,
      sub: `${counts.tasks.completed} completed`,
    },
    {
      label: "Team Members",
      value: counts.team.total,
      sub: `${counts.team.active} active`,
    },
    {
      label: "High-Priority Items",
      value: counts.highPriority,
      sub: "across backlogs & tasks",
      accent: true,
    },
  ];
  ...
  const tasksByStatus = COLUMNS.map((c) => ({
    label: c.title,
    count: counts.tasks.byStatus[c.id] ?? 0,
  }));
      ...
      <div className="metrics-grid">
        {summary.map((s) => (
          <div key={s.label} className="metric-card">
            ...
            <span className={`metric-value${s.accent ? " metric-accent" : ""}`}>
              {s.value}
            </span>
